...
export default function Produtos({estoqueSelecionado}:ProdutoProps){
  ...
  const [alertaProdutos, setAlertaProdutos] = useState<AlertaProdutos>({}as AlertaProdutos)
  ...
  useEffect(()=>{
    async function loadProducts(){
      ...
      const formattedProducts = response.data.map((product:Produto)=>{
        ...
        const formattedExpires = new Date(product.expires).toLocaleDateString('pt-br')
        return {
          ...product,
          formattedExpires,
          expireStatus
  ...
  return(
    <div id="page-produtos">
      ...
      <Col md={12}>
          {(alertaProdutos.proximoVencimento>0)&&<p className="alerta-amarelo">{alertaProdutos.proximoVencimento} itens próximos ao vencimento</p>}
          </Col>
          <Col md={12}>
          {(alertaProdutos.vencidos>0)&&<p className="alerta-vermelho">{alertaProdutos.vencidos} itens vencidos</p>}
        </Col>
      </Row>
      <Row>
        <Col>
            ...
            <tbody>
              {
                produtos.map(produto=>(
                  <tr className="linha-produto" key={produto.id}>
                      ...
                      <td className={`cell-expires ${produto.expireStatus}`}>{produto.formattedExpires}</td>
